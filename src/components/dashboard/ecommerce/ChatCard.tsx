...
export default function ChatCard() {
  return (
    <div className="dashboard-box col-span-12 py-6 xl:col-span-4">
      <h4 className="mb-6 px-8 text-xl font-semibold text-base-content">Chats</h4>
          ...
          <div
            ...
            <div className="relative h-14 w-14 rounded-full overflow-hidden border border-base-300">
              ...
              <span
                className={`absolute right-0 bottom-0 h-3.5 w-3.5 rounded-full border-2 border-base-100 ${
                  chat.dot === 6 ? "bg-success" : chat.dot === 3 ? "bg-warning" : "bg-error"
                }`}></span>
            </div>
            ...
            <div className="flex flex-1 items-center justify-between">
              <div>
                ...
                <p>
                  <span className="text-sm text-base-content/70">{chat.text}</span>
                  <span className="text-xs"> . {chat.time}</span>
                </p>
              </div>
              {chat.textCount !== 0 && (
                <div className="flex h-6 w-6 items-center justify-center rounded-full bg-primary">
                  <span className="text-sm font-medium text-primary-content">{chat.textCount}</span>
                </div>
              )}
            </div>
          </div>
